...
const SubmitRealEstateScreen = () => {
  ...
  const submit = async () => {
    ...
    if (!token || token === 'mock_token') {
      Alert.alert('Sign in required', 'Log in to post a property listing.');
      return;
    }
    ...
    if (!coords) {
      Alert.alert(
        ...
        'Enter valid latitude and longitude so the listing appears on the map.',
      );
      return;
    }
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.border, paddingTop: insets.top + 10 }]}>
        <TouchableOpacity style={styles.backBtn} onPress={() => navigation.canGoBack() ? navigation.goBack() : navigation.reset({ index: 0, routes: [{ name: 'Main' }] })}>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        <Text style={[styles.hint, { color: colors.mutedForeground }]}>
          Requires login (Bearer). One image uses POST /api/upload; several use POST /api/upload/multiple.
        </Text>
        ...
        <Text style={[styles.coordHint, { color: colors.mutedForeground }]}>
          Paste a Google Maps share link to auto-fill. Use the property location, not city center.
        </Text>
        <TextInput
        ...
        <Text style={[styles.label, { color: colors.mutedForeground }]}>Price (number or label)</Text>
        <TextInput
          value={price}
          onChangeText={setPrice}
          placeholder="e.g. 450 or €450/mo"
          placeholderTextColor={colors.mutedForeground}
...
export default SubmitRealEstateScreen;
